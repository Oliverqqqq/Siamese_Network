...
def differential_evolution(fobj, 
    ...
    min_, max_ = np.asarray(bounds).T
    #get range to denormalize
    range_ = np.fabs(min_ - max_)
    #denormalization
    pop_denorm = min_ +pop * range_
    ...
    for i in range(maxiter):
        ...
        for j in range (popsize):
            ...
            #pick three distinct individuals
            ...
            trail = np.where(cross_points, mutant, pop[j])
            trail_denorm = min_ + trail * range_
            #evaluate the cost of the trail vector
            test = fobj(trail_denorm)
            if test < cost[j]:
                cost[j] = test
                pop[j] = trail
                if test < cost[best_idx]:
                    best_idx = j
                    best = trail_denorm
...
def task_3():
    ...
    #try difference population sizes and generations
    budget = [(5,40),(10,20),(20,10),(40,5)]
    
    #create list to store result
    result = []
    #create loop to run all budget once
    for j in budget:
        de_gen = differential_evolution(
            eval_hyper, 
            bounds, 
    ...
    # Print the search result
        result.append('Stopped search after {} generation. Best accuracy reached is {}'.format(i,abs(c_w)))   
        result.append('Hyperparameters found:')
        result.append('nh1 = {}, nh2 = {}'.format(int(1+w[0]), int(1+w[1])))          
        result.append('alpha = {}, learning_rate_init = {}'.format(10**w[2],10**w[3]))
